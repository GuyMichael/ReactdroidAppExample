...
import com.guymichael.reactdroid.extensions.components.dialog.dialog
import com.guymichael.reactdroid.extensions.components.dialog.with
...
class MainPage(v: View) : ASimpleComponent<MainPageProps>(v) {
    ...
    private val cDialog = with(lazy { dialog(
            positiveBtn = R.string.dialog_btn_ok to {}
            , negativeBtn = null
            , title = R.string.home_dialog_title
            , message = R.string.home_dialog_msg
        )}
        , onDismiss = { MainStore.dispatch(GeneralReducerKey.welcomeDialogShown, false) }
    )

    override fun render() {
        cDialog.onRender(props.welcomeDialogShown)
    }
